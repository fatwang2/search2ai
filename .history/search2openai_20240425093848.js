    ...
    try {
      ...
      switch (SEARCH_SERVICE) {
        ...
        case "duckduckgo":
          ...
          break;
        case "searXNG":
          const searXNGBaseUrl = SEARXNG_BASE_URL;
          const searXNGResponse = await fetch(`${searXNGBaseUrl}/search?q=${encodeURIComponent(query)}&category=general&format=json&engines=bing&language=en-US&count=${MAX_RESULTS}`);
        default:
          console.error(`不支持的搜索服务: ${SEARCH_SERVICE}`);
          return `不支持的搜索服务: ${SEARCH_SERVICE}`;
      }
